...
% USAGE:
%
%    [mtxCoactive, mtxActivityX, mtxActivityY] = coactivityMatrix(cellSeqsX, <cellSeqsY>)
%
% DESCRIPTION:
%
...
%
%       The activity matrices (i.e., the return variables of `toMatrix`) of the
%       input arguments since they have to be computed as an intermediate step
%       of this computation.
%
function [mtxCoactive, mtxActivityX, mtxActivityY] = coactivityMatrix(cellSeqsX, cellSeqsY)
    if nargin < 2
        cellSeqsY = cellSeqsX;
    end

    nNeurons = maxActive([cellSeqsX(:); cellSeqsY(:)]);
    mtxActivityX = toMatrix(cellSeqsX, nNeurons);
    mtxActivityY = toMatrix(cellSeqsY, nNeurons);
