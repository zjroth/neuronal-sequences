% [UpCrossings DownCrossings] = SchmittTrigger(Signal,UpThresh,DownThresh)
%
...
% originally Ken's .. number of potentially usefull improvements by Anton
function [UpCross DownCross] = SchmittTrigger_e(Signal,UpThresh,DownThresh)
    ...
    UpCross = UpDownSort(PrevType==-1 & TypeSort==1);
    DownCross = UpDownSort(find(PrevType==-1 & TypeSort==1)+1);
    if(TypeSort(1) == 1)
        UpCross = [UpDownSort(1); UpCross];
        DownCross = [UpDownSort(2); DownCross];
    end
end